            ...
            <thead>
              <tr>
                ...
                <th>
                  ...
                  <a class="mx-auto" href="{{ route('print', ['month' => $_month['x']->format('m.Y')]) }}" target="_blank">
                    Ispis
                  </a>
                </th>
                <th class="w-32">
              ...
              <tr>
                ...
                <td>
                  <div class="w-full rounded-md relative {{$day->state == 4 ? 'bg-red' : ($day->state == 2 ? 'bg-green' : ($day->state == 3 ? 'bg-gray' : ($day->state == 1 ? 'bg-indigo' : 'bg-yellow')))}}-{{$day->date->format('D') == 'Sun' ? '300' : '100'}}" style="min-height: 18px;" title={{$day->date->format('d.m.Y')}}>
                    <div class="absolute rounded-l-md bg-indigo-700 min-h-full" style="width: {{($day->night ? $day->night->hour*60 + $day->night->minute : 0)/1440*100}}%;"></div>
                    <div class="absolute bg-indigo-500 min-h-full" style="margin-left: {{($day->start ? $day->start->hour*60 + $day->start->minute : 0)/1440*100}}%; width: {{($day->end ? $day->end->diffInMinutes($day->start) : 0)/1440*100}}%;"></div>
                  </div>
                </td>
                <td>
                  ...
                  <a class="float-right" style="color:red" href="{{ route('day.destroy', ['date' => $day->date->format('d.m.Y')]) }}" onclick="event.preventDefault();
