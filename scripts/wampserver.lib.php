...
function switchPhpVersion($newPhpVersion)
{
	...
	// eg: "$c_phpVersionDir = Wampee-dir\bin\php"
    ...
    // Save the current php.ini
    // eg: "$c_phpVersionDir = Wampee-dir\bin\php"
    // eg: "$c_apacheVersionDir = Wampee-dir\bin\apache"
    // eg: "$c_phpConfFile = Wampee-dir\bin\apache\apache'.$wampConf['apacheVersion'].'\'.$wampConf['apacheExeDir'].'\'.$wampConf['phpConfFile']"

    //copy($c_phpConfFile,$c_phpVersionDir.'/php'.$wampConf['phpVersion'].'/'.$phpConfFileForApache);
    
    // Copy new php.ini from template phpForApache.ini 
    copy($c_phpTplVersionDir.'/php'.$newPhpVersion.'/'.$phpConfFileForApache,$c_phpConfFile);

	// Replace path
	$phpIniFileContents = @file_get_contents($c_phpConfFile) or die ("php.ini file not found");
	$phpIniFileContents = str_replace("@PATH_WAMP@", $wampConf['installDir'], $phpIniFileContents);
	file_put_contents($c_phpConfFile, $phpIniFileContents);
    ...
    // Save the current php.ini
    copy($c_phpConfFile,$c_phpVersionDir.'/php'.$newPhpVersion.'/'.$phpConfFileForApache);
    copy($c_phpConfFile,$c_phpVersionDir.'/php'.$newPhpVersion.'/'.$wampConf['phpConfFile']);
